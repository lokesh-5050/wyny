...
exports.order = async (req, res, next) => {
  if (req.body.paymentMode === "COD") {
    ...
    for (
      var index = 0;
      index < showOrder.order[index].user[index].cart.length;
      index++
    ) {
      codTotAmount +=
        showOrder.order[index].user[index].cart[index].foods.price *
        showOrder.order[index].user[index].cart[index].quan;
    }
